...
long mk_cust(long n_cust, customer_t *c)
{
        long i;
        c->custkey = n_cust;
	sprintf(c->name, C_NAME_FMT, C_NAME_TAG, (int)n_cust);
...
#ifndef SUPPORT_64BITS
 #error Find a way to support_64bits! you know you can!
#endif
...
mk_order(long index, order_t *o, long upd_num)
	...
	for (lcnt = 0; lcnt < o->lines; lcnt++)
		...
		o->lineorders[lcnt].supp_cost = 0.6 * rprice;
...
mk_time(long index, dss_time_t *t)
	{
		...
		while (d > months[m].dcnt + LEAP_ADJ(y, m))
			m++;
		PR_DATE(t->alpha, (int)y, (int)m,
			(int)(d - months[m - 1].dcnt - ((LEAP(y) && m > 2) ? 1 : 0)));
		t->year = 1900 + y;
...
int gen_color(char * source, char * dest){
  ...
  dest[i]='\0';
  int clen = strlen(dest);
  int nlen = strlen(source);
  char *name;
  name = (char *) malloc(nlen-clen+2);
  strcpy(name, source+clen+1); // color len + space
  strcpy(source,name);
  free(name);
  return clen;
}
...
mk_date(long index,date_t *d)
{
    ...
    long numDateTime = D_STARTDATE + espan; 
